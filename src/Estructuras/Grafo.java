 ...
 */
public class Grafo {
	public static final int MAXVERT=999;
    ...
    private int[][] walsa = new int[MAXVERT][MAXVERT];   
    ...
    */
    ...
    /**
     * Metodo que devuelve el camino minimo entre 2 nodos
     * @param origen es el primer nodo
     * @param destino es el segundo nodo
     * @return Un Array con los nodos por los que pasa el camino minimo
     */
    public Queue<Integer> caminoMinimo(int origen, int destino){
  	  	int siguiente = origen;
  	  	Queue<Integer> Caminos = new LinkedList<Integer>();
  	  	Caminos.add(origen);
  	  	while (siguiente != destino){
  	  		siguiente = this.siguiente(siguiente, destino);
  	  		Caminos.add(siguiente);
  	  	}
  	  	return Caminos;
    }
    /**
    ...
    */
    ...
    public void contarNodos1(){
        ...
        System.out.println("Numero de nodos 1: " + count);
    }
    ...
    */
     ...
     * @return No retorna ningun valor
     */
     public void adyacentes(int origen, ArrayList<Integer> ady){
        if ((origen >= 0) && (origen < numNodos)) {
     		for (int i=0;i<numNodos;i++) {
            	 	if ((arcos[origen][i]!=INFINITO) && (arcos[origen][i]!=0))	
               		ady.add(i);	
     ...
     */
     ...
     public void mostrarguay()
     {
         int x,y;

         System.out.println("warshallguay:");
         for (x=0;x<numNodos;x++) {
             for (y=0;y<numNodos;y++){
                 System.out.print(this.walsa[x][y]);
                 System.out.print(" ");
             }    
             System.out.println();
         }
...
//     public void warshallguay() {
//         for (int k=0;k<numNodos;k++)
//             for (int i=0;i<numNodos;i++)
//                 for (int j=0;j<numNodos;j++){
//                	 if((warshal[i][j]<warshal[i][k])&&(warshal[i][j]<warshal[k][j]))
//                		 walsa[i][j]=warshal[i][j];
//                	 if((warshal[i][k]<warshal[i][j])&&(warshal[i][k]<warshal[k][j]))
//                		 walsa[i][j]=warshal[i][k];
//                	 if((warshal[k][j]<warshal[i][k])&&(warshal[k][j]<warshal[i][j]))
//                		 walsa[i][j]=warshal[k][j];
//                 }
//         			
//     }
     /**
     ...
     */
     ...
     /**
      * Crea camino minimo en el grafo,no quedan introducidos ni el origen ni el destino, por lo que habria que
        introducir el origen antes y el destino despues, no problem at all.
      * 
      * @param origen primera casilla en la que se encuentra
      * @param destino casilla objetivo
      * @param cami camino que queremos obtener
      * @return del camino
      */
     public void CaminoMinimo(int origen,int destino, Queue<Integer> cami){
    	 int aux=this.floydP[origen][destino];
    	 //System.out.println(origen);
    	 if(aux!=0&&aux!=INFINITO){
    		 CaminoMinimo(origen,aux,cami);	 
    		 cami.add(aux);
    		 //System.out.print(aux+" ");
    		 CaminoMinimo(aux,destino,cami);
    	 }
    	 //return cami;
       }
      ...
      */
      ...
      /**
       * Implementacion de recorrido en profundidad del grafo
       * @param v es el nodo origen
       */
      public void profundidad(int v){
    	  ArrayList<Integer> Visitados = new ArrayList<Integer>();
    	  prof(v, Visitados);
      }
	
      private void prof(int v, ArrayList<Integer> Visitados){
...
//    	  System.out.println(v);
	...
	// -----------------------------------------------------------------------------------------------------------------
	    ...
	    //System.out.println(g.floyd().length);
	    //g.warshallguay();
	    //g.CaminoMinimo(0, 1);
	}
	*/
